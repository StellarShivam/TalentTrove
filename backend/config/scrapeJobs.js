...
const createNaurkiScrapeLinks = (jobCategories) => {
  ...
  for (var i = 0; i < jobCategories.length; i++) {
    for (var j = 1; j <= 2; j++) {
      ...
      let url = `https://www.naukri.com/${string1}?k=${string2}&nignbevent_src=jobsearchDeskGNB`;
      categoryLinks.push({ url, jobCategory: jobCategories[i] });
    }
  }
  return categoryLinks;
...
const scrapeIndeedPage = async (job, page) => {
  try {
    ...
    const posted = $(
      "#mosaic-provider-jobcards > ul > li > div > div > div > div > div > table.css-1u8dvic.eu4oa1w0 > tbody > tr > td > div.heading6.tapItem-gutter.css-193h767.eu4oa1w0 > span.css-qvloho.eu4oa1w0"
    )
      .map((i, element) => {
...
const scrapeNaukriPage = async (url, page) => {
  try {
...
const scrapeNaukriDescriptionPage = async (url, page) => {
  try {
    await page.goto(url, { waitUntil: "networkidle2" });
    const html = await page.evaluate(() => document.body.innerHTML);
    const $ = await cheerio.load(html);
